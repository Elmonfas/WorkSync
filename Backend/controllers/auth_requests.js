...
const get_requests = async (req, res) => {
    ...
    try {
        ...
        const requests = await con.query('SELECT request_id, request_date, requested_day,DAYNAME(requests.requested_day) AS day_of_week , message FROM requests JOIN Users ON user_id = Users.ID WHERE Users.ID = ?', [req.uid])
...
const create_request = async (req, res) => {
    ...
    try {
        ...
        const {request_date, requested_day, message} = req.body
